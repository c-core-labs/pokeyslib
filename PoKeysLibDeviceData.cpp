...
int PK_DeviceDataGet(sPoKeysDevice* device)
{
	...
	bool devSeries55 = false;
	bool devSeries56 = false;
	bool devSeries27 = false;
	bool devUSB = false;
	bool devEth = false;
	bool devBootloader = false;
	...
	if (SendRequest(device) == PK_OK)
    {
		for (int i = 0; i < 4; i++)	data->BuildDate[i + 0] = device->response[2 + i];
	} else return PK_ERR_TRANSFER;

    CreateRequest(device->request, 0x04, 1, 0, 0, 0);
	if (SendRequest(device) == PK_OK)
    {
		for (int i = 0; i < 4; i++)	data->BuildDate[i + 4] = device->response[2 + i];
	} else return PK_ERR_TRANSFER;

    CreateRequest(device->request, 0x04, 2, 0, 0, 0);
	if (SendRequest(device) == PK_OK)
    {
		for (int i = 0; i < 3; i++)	data->BuildDate[i + 8] = device->response[2 + i];
		data->BuildDate[11] = 0;
	} else return PK_ERR_TRANSFER;
	...
	if (!devBootloader)
	{
		...
		if (true)						info->iAnalogInputs = 1;
		if (true)						info->iAnalogFiltering = 1;
