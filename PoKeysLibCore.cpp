...
void InitializeNewDevice(sPoKeysDevice* device)
{
	...
	device->Pins = new sPoKeysPinData[device->info.iPinCount];
	memset(device->Pins, 0, sizeof(sPoKeysPinData) * device->info.iPinCount);

	for (int i = 0; i < device->info.iPinCount; i++)
	{
	...
	device->Encoders = new sPoKeysEncoder[device->info.iEncodersCount];
	memset(device->Encoders, 0, sizeof(sPoKeysEncoder) * device->info.iEncodersCount);

	device->PWM.PWMduty = new uint32_t[device->info.iPWMCount];
	memset(device->PWM.PWMduty, 0, sizeof(uint32_t) * device->info.iPWMCount);

	device->PWM.PWMenabledChannels = new unsigned char[device->info.iPWMCount];
	memset(device->PWM.PWMenabledChannels, 0, sizeof(unsigned char) * device->info.iPWMCount);

	device->PoExtBusData = new unsigned char[device->info.iPoExtBus];

	device->MatrixLED = new sPoKeysMatrixLED[device->info.iEncodersCount];
	memset(device->MatrixLED, 0, sizeof(sPoKeysMatrixLED) * device->info.iMatrixLED);

	if (device->info.iPulseEngine)
	{
		device->PulseEngine = new sPoKeysPE();

		PK_PEInfoGet(device);

		// Allocate buffer
		device->PulseEngine->buffer.buffer = new unsigned char[device->PulseEngine->info.bufferDepth];
		memset(device->PulseEngine->buffer.buffer, 0, sizeof(unsigned char) * device->PulseEngine->info.bufferDepth);

		device->PulseEngine->ReferencePosition = new uint32_t[device->PulseEngine->info.nrOfAxes];
		memset(device->PulseEngine->ReferencePosition, 0, sizeof(uint32_t) * device->PulseEngine->info.nrOfAxes);
		device->PulseEngine->CurrentPosition = new uint32_t[device->PulseEngine->info.nrOfAxes];
		memset(device->PulseEngine->CurrentPosition, 0, sizeof(uint32_t) * device->PulseEngine->info.nrOfAxes);
		device->PulseEngine->MaxSpeed = new uint32_t[device->PulseEngine->info.nrOfAxes];
		memset(device->PulseEngine->MaxSpeed, 0, sizeof(uint32_t) * device->PulseEngine->info.nrOfAxes);
		device->PulseEngine->MaxAcceleration = new uint32_t[device->PulseEngine->info.nrOfAxes];
		memset(device->PulseEngine->MaxAcceleration, 0, sizeof(uint32_t) * device->PulseEngine->info.nrOfAxes);
		device->PulseEngine->MaxDecceleration = new uint32_t[device->PulseEngine->info.nrOfAxes];
		memset(device->PulseEngine->MaxDecceleration, 0, sizeof(uint32_t) * device->PulseEngine->info.nrOfAxes);
		device->PulseEngine->AxisState = new unsigned char[device->PulseEngine->info.nrOfAxes];
		memset(device->PulseEngine->AxisState, 0, sizeof(unsigned char) * device->PulseEngine->info.nrOfAxes);

		device->PulseEngine->MPGjogMultiplier = new uint32_t[device->PulseEngine->info.nrOfAxes];
		memset(device->PulseEngine->MPGjogMultiplier, 0, sizeof(uint32_t) * device->PulseEngine->info.nrOfAxes);
		device->PulseEngine->MPGaxisEncoder = new unsigned char[device->PulseEngine->info.nrOfAxes];
...
{
	delete device->Pins;
	delete device->Encoders;
	delete device->PWM.PWMduty;
	delete device->PWM.PWMenabledChannels;
	delete device->PoExtBusData;
	delete device->MatrixLED;
	if (device->PulseEngine != NULL)
	{
		delete device->PulseEngine->buffer.buffer;
		
		delete device->PulseEngine->ReferencePosition;
		delete device->PulseEngine->CurrentPosition;
		delete device->PulseEngine->MaxSpeed;
		delete device->PulseEngine->MaxAcceleration;
		delete device->PulseEngine->MaxDecceleration;
		delete device->PulseEngine->AxisState;

		delete device->PulseEngine->MPGjogMultiplier;
		delete device->PulseEngine->MPGaxisEncoder;
		delete device->PulseEngine;
	}
}
...
sPoKeysDevice* PK_ConnectToDeviceWSerial(long serialNumber, int checkForNetworkDevicesAndTimeout)
{
	...
	{
		sPoKeysNetworkDeviceSummary * devices = new sPoKeysNetworkDeviceSummary[16];
